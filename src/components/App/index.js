...
const App = () => {
  ...
  const handleButtonClick = (rowParam, colParam) => e => {
    ...
    // если наткнулись на бомбу - игра окончена
    if (cell.value === -1) {
      setHasLost(true);
    ...
    // если все кнопки без бомб нажаты
  ...
  const handleButtonContextMenu = (rowParam, colParam) => e => {
    ...
    // возвращает значение, если уже видмо
    if (cell.state === 1) {
      return;
    }

    // если нет флага, добавляем
    if (cell.state === 0) {
